...
class Lekeres {
	...
	private function db_processor($go) {
		...
		*/
		...
		while ($row = mysqli_fetch_array($result) ) {
			...
			$groupID = (( ! empty($row["db1"]) ) ? $row["db1"] : "" ); // . ( (! empty($row["db3"]) ) ? "inParanoid". "-" : "" );
				...
				foreach ($sorrend as $nr => $this_key) {
					...
					foreach ($grouping as $groupID => $unipX) {
						...
						foreach ($ListOfAll as $unip1 => $faj1) {
							...
							if(! array_key_exists($this->unip2groupID[$unip1], $ListOfGroups)) $ListOfGroups[$this->unip2groupID[$unip1]] = array($faj1 => array($unip1 => ((array_key_exists($unip1, $this->protein_list[$fajKeys_mid2lil[$faj1]])) ? "<B>".$unip1."</B>" : $unip1 )));
							if(! array_key_exists($faj1, $ListOfGroups[$this->unip2groupID[$unip1]])) $ListOfGroups[$this->unip2groupID[$unip1]][$faj1] = array($unip1 => ((array_key_exists($unip1, $this->protein_list[$fajKeys_mid2lil[$faj1]])) ? "<B>".$unip1."</B>" : $unip1 ));
							else $ListOfGroups[$this->unip2groupID[$unip1]][$faj1][$unip1] = ((array_key_exists($unip1, $this->protein_list[$fajKeys_mid2lil[$faj1]])) ? "<B>".$unip1."</B>" : $unip1 );
	...
	public function CreateTable(){
		...
		foreach ($this->containertable as $this_key => $groups) {
			...
			foreach ($this_keys as $k => $v) $table .= "<TD><B>".$this->faj[strtolower($v)]["long"]."</B></TD>\n";
			$table .= "</TR>\n";

			foreach ($groups as $groupID => $arri2) {

				$row = "<TR>\n";
				$row .= "<TD width=\"" . (100/(count($this_keys)+1)) . "%\">$groupID</TD>\n";
				...
				foreach ($this_keys as $k => $v) {
					...
					else $row .= implode(", ", $groups[$groupID][$this->faj[strtolower($v)]["mid"]]) . "</TD>\n";

				}

				$row .= "</TR>\n";

				if( ! strpos($row, "n/a") ) $table .= $row;
