...
class Lekeres {
	...
	public function __construct($files, $folders, $faj, $given_values) {
		...
		foreach($faj as $key => $arri) {
			...
			$this->original_lista[ $arri["lil"] ] = array();
			$this->mappings[ $arri["mid"] ] = array();
			$this->faj_mid2lil[ $arri["mid"] ] = $arri["lil"];
		}

		$this->kiir_fajl = $files["output"];
		$this->lista = self::file_processor($files["gos"][$this->get_values["go"]]);
		...
		while (($sor_tartalom = fgets($fajl_beolvas)) !== false) {
			...
			if ( empty($sor_tartalom) || ( $sor == 1 && !$isList ) ) continue;
				...
				elseif($isList) {
					...
					if(! array_key_exists($unip1, $this->original_lista[ $this->faj_mid2lil[$faj1] ])) $this->original_lista[ $this->faj_mid2lil[$faj1] ][$unip1] = $unip1;
				}
